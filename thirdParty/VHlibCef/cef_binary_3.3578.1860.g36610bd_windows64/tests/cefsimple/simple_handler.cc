...
bool SimpleHandler::Execute(const CefString& name,
   ...
   if (name == "JsCallQtMsg" || name == "JsCallQtOpenUrl" || name == "JsCallQtStartLive" || 
      ...
      name == "JsCallQtStartVoiceLive" || name == "JsCallQtMsg" || name == "JsCallFadeOutTip") {
   ...
   else if (name == "JsCallQtJoinActivity" || name == "ClientCallCloseExamCallback" || name == "sendQtCloseSuccess" || name == "sendQtStopAnswer") {
      CefRefPtr<CefProcessMessage> msg = CefProcessMessage::Create(name);
      CefV8Context::GetCurrentContext()->GetBrowser()->SendProcessMessage(PID_BROWSER, msg);
   }
